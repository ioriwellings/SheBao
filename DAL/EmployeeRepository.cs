...
namespace Langben.DAL
{
    ...
    public partial class EmployeeRepository : BaseRepository<Employee>, IDisposable
    {
        ...
        /// <returns></returns>      
        public IQueryable<Employee> GetData(SysEntities db, string order, string sort, string search, params object[] listQuery)
        {
            ...
            return ((System.Data.Entity.Infrastructure.IObjectContextAdapter)db).ObjectContext 
                     .CreateObjectSet<Employee>().Where(string.IsNullOrEmpty(where) ? "true" : where)
                     .OrderBy("it.[" + sort.GetString() + "] " + order.GetString())
                     .AsQueryable(); 

        }
        /// <summary>
        /// 通过主键id，获取员工---查看详细，首次编辑
        /// </summary>
        ...
        /// <param name="id">主键</param>
        /// <returns>员工</returns>
        public Employee GetById(SysEntities db, int id)
        { 
            return db.Employee.SingleOrDefault(s => s.Id == id);
        ...
        /// <param name="id">一条员工的主键</param>
        public void Delete(SysEntities db, int id)
        {
            Employee deleteItem = GetById(db, id);
            if (deleteItem != null)
            { 
                db.Employee.Remove(deleteItem);
            }
        }
        /// <summary>
        /// 删除对象集合
        ...
        {
            //数据库设置级联关系，自动删除子表的内容   
            IQueryable<Employee> collection = from f in db.Employee
