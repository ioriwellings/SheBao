...
namespace Langben.DAL
{
    ...
    public partial class ORG_MenuRepository : BaseRepository<ORG_Menu>, IDisposable
    {
        ...
        /// <returns></returns>      
        public IQueryable<ORG_Menu> GetData(SysEntities db, string order, string sort, string search, params object[] listQuery)
        {
            ...
            return ((System.Data.Entity.Infrastructure.IObjectContextAdapter)db).ObjectContext 
                     .CreateObjectSet<ORG_Menu>().Where(string.IsNullOrEmpty(where) ? "true" : where)
                     .OrderBy("it.[" + sort.GetString() + "] " + order.GetString())
                     .AsQueryable(); 

        }
        /// <summary>
        /// 通过主键id，获取组织机构_菜单---查看详细，首次编辑
        /// </summary>
        /// <param name="id">主键</param>
        /// <returns>组织机构_菜单</returns>
        ...
        /// <param name="id">主键</param>
        /// <returns>组织机构_菜单</returns>
        public ORG_Menu GetById(SysEntities db, int id)
        { 
            return db.ORG_Menu.SingleOrDefault(s => s.ID == id);
        ...
        /// <param name="id">一条组织机构_菜单的主键</param>
        public void Delete(SysEntities db, int id)
        {
            ORG_Menu deleteItem = GetById(db, id);
            if (deleteItem != null)
            { 
                db.ORG_Menu.Remove(deleteItem);
            }
        }
        /// <summary>
        /// 删除对象集合
        ...
        {
            //数据库设置级联关系，自动删除子表的内容   
            IQueryable<ORG_Menu> collection = from f in db.ORG_Menu
