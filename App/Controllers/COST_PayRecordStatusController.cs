...
namespace Langben.App.Controllers
{
    ...
    /// 费用_社保支出导入汇总
    /// </summary>
    public class COST_PayRecordStatusController : BaseController
    {
        ...
        public JsonResult GetData(int? id, int page, int rows, string order, string sort, string search)
        {
            ...
            int total = 0;
            List<COST_PayRecordStatus> queryData = m_BLL.GetByParam(id, page, rows, order, sort, search, ref total);
            return Json(new datagrid
            {
                ...
                rows = queryData.Select(s => new
                {
					...
					,CostType = s.CostType
					,Status = s.Status
					,COST_PayRecordId = s.COST_PayRecordId
        ...
        public ActionResult Export(string id, string title, string field, string sortName, string sortOrder, string search)
        {
            ...
            List<COST_PayRecordStatus> queryData = m_BLL.GetByParam(id, sortOrder, sortName, search);
        ...
        [HttpPost]
        [SupportFilter]
        public ActionResult Create(COST_PayRecordStatus entity)
        {           
            if (entity != null && ModelState.IsValid)
            {
                ...
                {
                    LogClassModels.WriteServiceLog(Suggestion.InsertSucceed  + "，费用_社保支出导入汇总的信息的Id为" + entity.ID,"费用_社保支出导入汇总"
                        );//写入日志 
                ...
                { 
                    ...
                    LogClassModels.WriteServiceLog(Suggestion.InsertFail + "，费用_社保支出导入汇总的信息，" + returnValue,"费用_社保支出导入汇总"
                        );//写入日志                      
        ...
        IBLL.ICOST_PayRecordStatusBLL m_BLL;

        ValidationErrors validationErrors = new ValidationErrors();

        public COST_PayRecordStatusController()
            : this(new COST_PayRecordStatusBLL()) { }

        public COST_PayRecordStatusController(COST_PayRecordStatusBLL bll)
        {
            m_BLL = bll;
        }
