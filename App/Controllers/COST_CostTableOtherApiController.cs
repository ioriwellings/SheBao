...
namespace Langben.App.Controllers
{
    ...
    /// 费用_费用表其他费用
    /// </summary>
    public class COST_CostTableOtherApiController : BaseApiController
    {
        /// <summary>
        /// 异步加载数据
        /// </summary>
        ...
        /// <returns></returns>  
        public Common.ClientResult.Result Put([FromBody]COST_CostTableOther entity)
        {
            Common.ClientResult.Result result = new Common.ClientResult.Result();
            if (entity != null && ModelState.IsValid)
            {   //数据校验
                ...
                {
                    LogClassModels.WriteServiceLog(Suggestion.UpdateSucceed + "，费用_费用表其他费用信息的Id为" + entity.ID,"费用_费用表其他费用"
                        );//写入日志                   
                    result.Code = Common.ClientCode.Succeed;
                ...
                else
                {
                    ...
                    LogClassModels.WriteServiceLog(Suggestion.UpdateFail + "，费用_费用表其他费用信息的Id为" + entity.ID + "," + returnValue, "费用_费用表其他费用"
                        );//写入日志   
                    result.Code = Common.ClientCode.Fail;
        ...
        IBLL.ICOST_CostTableOtherBLL m_BLL;

        ValidationErrors validationErrors = new ValidationErrors();

        public COST_CostTableOtherApiController()
            : this(new COST_CostTableOtherBLL()) { }

        public COST_CostTableOtherApiController(COST_CostTableOtherBLL bll)
        {
            m_BLL = bll;
        }
